...
# Ensure commands are registered to the correct guild
@bot.event
async def on_ready():
...
@app_commands.checks.has_role(WHITELIST_ROLE_ID)
@app_commands.describe(route="Flight route (e.g., Manchester to Alicante)", start_time="Start time (DD/MM/YYYY HH:MM)", aircraft="Aircraft (e.g., B737-800)", flight_code="Flight code (e.g., LS8800)")
async def flight_create(interaction: discord.Interaction, route: str, start_time: str, aircraft: str, flight_code: str):
    try:
        ...
        event = await guild.create_scheduled_event(
            ...
            image=await bot.http.get(EVENT_BANNER_URL),
